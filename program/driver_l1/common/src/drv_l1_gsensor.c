...
//	DBG_PRINT("chip_id = %x\r\n", temp1);
	...
	g_sensor_write(G_SlaveAddr,INT_CONFIG,0x05);
...
{
    INT16U temp=0xff,temp1=0xff; 
	INT16U result=0;
	sw_i2c_lock();
#if (USE_G_SENSOR_NAME == G_SENSOR_DA380)
	temp1 = g_sensor_read(G_SlaveAddr, ACC_Y_MSB);
	result=temp1;

	temp = g_sensor_read(G_SlaveAddr, ACC_Y_LSB);///CHIPID   MOTION_FLAG
	
	result=result*256+temp;
	//temp2 = g_sensor_read(G_SlaveAddr, ACC_Y_LSB);
	//temp3 = g_sensor_read(G_SlaveAddr, ACC_Y_MSB);
	//temp4 = g_sensor_read(G_SlaveAddr, ACC_Z_LSB);
	//temp5 = g_sensor_read(G_SlaveAddr, ACC_Z_MSB);
	//DBG_PRINT("xl=%x,xh=%x,yl=%x,yh=%x,zl=%x,zh=%x\r\n",temp,temp1,temp2,temp3,temp4,temp5);
	//DBG_PRINT("yl=%x,yh=%x,re=%x\r\n",temp,temp1,result);
#elif (USE_G_SENSOR_NAME == G_SENSOR_DMARD07)
