...
static INT8U    sensor_id_check=0;
...
BATTERY_Lv0 = 2035 * BATTERY_CNT
};

enum
{
  SENSOR_1_L =45000,//30000,//30000,//36000
  SENSOR_1_H= 65535, //60000,//54000
  SENSOR_0_L =100,//5800,//9800
  SENSOR_0_H= 6000//29000//19000//15000
};
...
void ap_peripheral_key_init(void)
{
	...
	gpio_write_io(SWITCH_KEY, 0);
	
	gpio_init_io(PIR_MODE, GPIO_INPUT);		
	gpio_set_port_attribute(PIR_MODE, ATTRIBUTE_LOW);	
	gpio_write_io(PIR_MODE, 0);
...
static void mode_video_key_handle(void)
{
...
static void mode_midtion_key_handle(void)
{
	if(video_record_sts&0x02)
	{
		msgQSend(ApQ, MSG_APQ_VIDEO_RECORD_ACTIVE, NULL, NULL, MSG_PRI_NORMAL);
	...
	{
		__msg("motion sensor detect\n");
		msgQSend(ApQ, MSG_APQ_MD_SET, NULL, NULL, MSG_PRI_NORMAL);
	}
}

static void mode_power_off_handle(void)
{
	if(video_record_sts & 0x04)		//痄雄淈聆
	{
		if((video_record_sts & 0x02))
	...
	else if(video_record_sts&0x02)
	{
		msgQSend(ApQ, MSG_APQ_VIDEO_RECORD_ACTIVE, NULL, NULL, MSG_PRI_NORMAL);
	}
	if (usb_state_get() == 0)
	{
		__msg("power_off high\n");
		msgQSend(ApQ, MSG_APQ_POWER_KEY_ACTIVE, NULL, NULL, MSG_PRI_NORMAL);
	}
}

static INT8U  current_state=0;
static INT8U  prev_state=0xff;
static INT8U  delay_sec=0;
static INT8U  current_state_temp=0;
static INT8U  check_cnt=0;


void ap_peripheral_key_judge(void)
{
#if 1
	INT16U			power_key = 0, record_key = 0;
	if(++delay_sec>12)
	{
		power_key = sys_pwr_key0_read();
		record_key = gpio_read_io(SWITCH_KEY);

		current_state=(INT8U)((power_key<<7)|record_key);
		//__msg("p:%d, r: %d, c: %d\n", power_key, record_key, current_state);

		if(current_state_temp == current_state)
		{
			check_cnt++;
		}
		else
		...
		if((current_state !=prev_state)&&check_cnt>10)
		{
			check_cnt=0;
			if((1 == power_key)&&(1 == record_key))
			{
				__msg("all high level\n");
				mode_video_key_handle();
			}
			else if((1 == power_key)&&(0 == record_key))
			{
				__msg("record key low\n");
				mode_midtion_key_handle();
			}
			else if(0 == power_key)
			{
				__msg("power key low, power off\n");
				mode_power_off_handle();
			}
...
INT8U sensor_filp0_cnt=0;
INT8U sensor_filp1_cnt=0;
...
void ap_peripheral_read_gsensor(void)
{
	...
	DBG_PRINT("Read_gsen_value=0x%x\r\n", temp);

	if ((temp > SENSOR_0_L) && (temp < SENSOR_0_H))
	{
		sensor_filp0_cnt++;
		sensor_filp1_cnt	= 0;
	}
	else if ((temp > SENSOR_1_L) && (temp < SENSOR_1_H))
	{
		sensor_filp1_cnt++;
		sensor_filp0_cnt	= 0;
	}
	...
	if (sensor_filp != prev_sensor_filp)
	{
		prev_sensor_filp	= sensor_filp;

		//DBG_PRINT("fanzzzzzzzzzzzzzzzz\r\n");
		msgQSend(ApQ, MSG_APQ_SENSOR_FILP, NULL, NULL, MSG_PRI_NORMAL);
	}
