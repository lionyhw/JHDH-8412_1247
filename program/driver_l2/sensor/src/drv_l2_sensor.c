...
void sensor_frame_range_clip(sensor_frame_range_t* frameRange)
{
	...
	{	
		if (sensor_filp) {//sensor_filp  SENSOR_FLIP//if (SENSOR_FLIP) 
	 ...
	 INT32U Wfactor, Hfactor; 
			...
			{		  
					if (sensor_filp) { 
							...
							hwCdsp_SetRawDataFormat(2);  
					} 
			} 
        #endif 
        #if (sensor_format == SOI_H22_MIPI)  
			{		  
					if (sensor_filp) { 
							sccb_write(SOI_H22_ID, 0x12, 0x30); 	   // [0]mirror [1]flip 
							R_CDSP_MIPI_HVOFFSET = 0x03 & 0xFFF | (INT32U)(0x01& 0xFFF ) << 12; 												
					} 
					else { 
							sccb_write(SOI_H22_ID, 0x12, 0x00); 	   // [0]mirror [1]flip 
							R_CDSP_MIPI_HVOFFSET = 0x04 & 0xFFF | (INT32U)(0x02& 0xFFF ) << 12; 
					} 
			} 
        #endif 
#elif (USE_SENSOR_NAME == SENSOR_GC1004) 
			...
			{		  
					if (sensor_filp) { 
							sccb_write(GC1004_SLAVE_ID, 0x17, 0x17);		// [0]mirror [1]flip 
							//R_CDSP_FRAME_H_SETTING = (INT32U)(0x0C) << 16;		 
							//R_CDSP_FRAME_V_SETTING = (INT32U)(0x01) << 16;		 
							hwFront_SetFrameSize(0x0C, 0x01, 1280, 720);												 
					} 
					else { 
							sccb_write(GC1004_SLAVE_ID, 0x17, 0x14);		// [0]mirror [1]flip 
							//R_CDSP_FRAME_H_SETTING = (INT32U)(0x07) << 16;		 
							//R_CDSP_FRAME_V_SETTING = (INT32U)(0x02) << 16;		 
							hwFront_SetFrameSize(0x07, 0x02, 1280, 720); 
					} 
			} 
        #endif 
        #if (sensor_format == GC1004_MIPI)  
